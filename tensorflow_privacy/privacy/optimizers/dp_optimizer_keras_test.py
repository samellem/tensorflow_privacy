...
class DPVectorizedOptimizerUnconnectedNodesTest(tf.test.TestCase,
  ...
  unconnected layers and setting unconnected gradients to 'None' or 0.
  """
  ...
  def keras_embedding_model_fn(self,
                               opt_cls,
                               l2_norm_clip: float,
    ...
    sequence_output, pooled_output = self.SimpleEmbeddingModel()(x)
    if use_seq_output:
  ...
  def testSeqOutputUnconnectedGradientsAsNoneFails(self, cls):
    ...
    embedding_model = self.keras_embedding_model_fn(
        cls,
        l2_norm_clip=1.0,
  ...
  def testPooledOutputUnconnectedGradientsAsNonePasses(self, cls):
    ...
    try:
      ...
      self.fail('ValueError not raised by model.fit().')
